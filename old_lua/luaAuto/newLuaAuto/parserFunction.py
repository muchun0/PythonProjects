...
def checkLuaVersion(effectzip,effectLog):
    ...
    if('event.lua' in list_dir):
        ...
        if(LooseVersion(effectzip.version) < LooseVersion(version)):
            ...
            msg = 'error: config.json版本为 %s, line = %d function = %s 请提高版本到 %s'%(effectzip.version,line,function,version)
        ...
        if('event.lua' in list_dir):
            ...
            with open(lua_path, 'r') as f:
                ...
                try:
                    ...
                    msg ="file:%s event.lua particple Error"%(effectzip.zip_name)
                    effectLog.addEnglishErrorThing(msg)
                functionList = ['getFeature','addFeature','getAudioManager','getRenderCacheTexture','renderCacheFaceInfo','getPhotoAlgorithmResultByBitIndex','getBgInfo','maskInfo','maskdata']
                i = 0
                while(i < len(stringList)):
    ...
    for di in effectzip.dirs:
        ...
        for disDir in disableDirs:
            if(disDir in temp_di):
                ...
                enmsg = 'error: effect has diable dir%s,If confirm file is invalid, please open vscode to delete'%(disDir)
                effectLog.addEnglishErrorThing(msg)
...
def checkFaceuVersion(effectzip,effectLog,lokiInformation):
    ...
    if LooseVersion(max_ios_version) > LooseVersion(appVersionIosMin):
        msg ='error:ios最低版本号Loki配置错误，%s能力最低版本号限制%s，请将%s修改至%s及以上'%(max_ios_version_effect,max_ios_version,appVersionIosMin,max_ios_version)
        enmsg = 'error:ios Minimum version number configuration error，the feature:%s Minimum version %s，Please Change ≥ %s'%(max_ios_version_effect,max_ios_version,max_ios_version)
        effectLog.addEnglishErrorThing(msg)
    ...
    if LooseVersion(max_android_version) > LooseVersion(appVersionAndroidMin):
        msg ='error:Android最低版本号Loki配置错误，%s能力最低版本号限制%s，请将%s修改至%s及以上'%(max_android_version_effect,max_android_version,appVersionAndroidMin,max_android_version)
        enmsg = 'error:Android Minimum version number configuration error，the feature:%s Minimum version %s，Please Change ≥ %s'%(max_android_version_effect,max_android_version,max_android_version)
        effectLog.addEnglishErrorThing(msg)
...
def checkQingyanVersion(effectzip,effectLog,lokiInformation):
    ...
    if LooseVersion(max_ios_version) > LooseVersion(appVersionIosMin):
        msg ='error:轻颜ios最低版本号Loki配置错误，%s能力最低版本号限制%s，请将%s修改至%s及以上'%(max_ios_version_effect,max_ios_version,appVersionIosMin,max_ios_version)
        enmsg = 'error:ios Minimum version number configuration error，the feature:%s Minimum version %s，Please Change ≥ %s'%(max_ios_version_effect,max_ios_version,max_ios_version)
        effectLog.addEnglishErrorThing(msg)
        effectLog.addJsonData(1,'Loki_appVersionIosMin_ERROR',effectzip.zip_name,-1,msg,enmsg)
    ...
    if LooseVersion(max_android_version) > LooseVersion(appVersionAndroidMin):
        msg ='error:轻颜Android最低版本号Loki配置错误，%s能力最低版本号限制%s，请将%s修改至%s及以上'%(max_android_version_effect,max_android_version,appVersionAndroidMin,max_android_version)
        enmsg = 'error:Android Minimum version number configuration error，the feature:%s Minimum version %s，Please Change ≥ %s'%(max_android_version_effect,max_android_version,max_android_version)
        effectLog.addEnglishErrorThing(msg)
        effectLog.addJsonData(1,'Loki_appVersionAndroidMin_ERROR',effectzip.zip_name,-1,msg,enmsg)
...
def checkXiguaPanelKey(effectzip,effectLog,lokiInformation):
    ...
    if (str(panelKey) == "propsindex"):
        if (XiaguaAttributes == "MIMO" or XiaguaAttributes == "MV"):
            msg ='error:Loki面板配置错误,该资源包类型为%s,[首页道具propsindex]只支持非Mimo和非MV格式的道具包'%(XiaguaAttributes)
            enmsg = 'error:Loki panel configuration error, the resource pack type is%s,propsindex only supports non-Mimo and non-MV'%(XiaguaAttributes)
            effectLog.addEnglishErrorThing(msg)
        ...
        if XiaguaAttributes != "MV":
            ...
            enmsg = 'error:Loki panel configuration error, the resource pack type is%s,prossynthesis only supports MV'%(XiaguaAttributes)
            effectLog.addEnglishErrorThing(msg)
            effectLog.addJsonData(1,'Loki_PanelKey_ERROR',effectzip.zip_name,-1,msg,enmsg)
    elif (str(panelKey)== "props"):
        if XiaguaAttributes == "MV":
            msg ='error:Loki面板配置错误,该资源包类型为%s,[拍摄道具props]只支持非MV格式的道具包'%(XiaguaAttributes)
            enmsg = 'error:Loki panel configuration error, the resource pack type is%s,props only supports non-MV'%(XiaguaAttributes)
            effectLog.addEnglishErrorThing(msg)
            effectLog.addJsonData(1,'Loki_PanelKey_ERROR',effectzip.zip_name,-1,msg,enmsg)
    elif (str(panelKey)== "shoot-props-test"):
        if XiaguaAttributes == "MV":
            msg ='error:Loki面板配置错误,该资源包类型为%s,[内测版拍摄道具test]只支持非MV格式的道具包'%(XiaguaAttributes)
            enmsg = 'error:Loki panel configuration error, the resource pack type is%s,shoot-props-test only supports non-MV'%(XiaguaAttributes)
            effectLog.addEnglishErrorThing(msg)
            effectLog.addJsonData(1,'Loki_PanelKey_ERROR',effectzip.zip_name,-1,msg,enmsg)
...
def check_pics_size(param,effectzip,effectLog,lokiInformation=None):
    ...
    if len(ret) == 0:
        ...
        return
                ...
                # for r in ret:
                #     print(r)
        msg = "%s png基准说明：大小为%sKB,分辨率为%s*%s;该资源包中不通过PNG详情如下%s，请按标准重新修改"%(param,max_size,pixela,pixelb,ret)
        enmsg = "%s PNG Standard: The size %sKB, The resolution %s*%s;the fail png list is%s please re-edit"%(param,max_size,pixela,pixelb,ret)
        effectLog.addEnglishErrorThing(msg)
        effectLog.addJsonData(1,'PNG_SIZE_ERROR',effectzip.zip_name,-1,msg,enmsg)
    """
End::------------------------
            ...
            if os.path.exists(config_path):
                ...
                if 'exclusiveScene' in config['effect']:
                    ...
                    if 'sceneKey' not in comp_dict:
                        ...
                        makeup_oppo.append('config.json下exclusiveScene字段格式错误，sceneKey={}'.format(config['effect']['exclusiveScene'][0]['sceneKey']))
                else:
            ...
            if os.path.exists(extra_ptah):
                ...
                if 'disableExtMakeup' in extra['settings']:
                    if extra['settings']['disableExtMakeup'] != 1:
                        makeup_oppo.append('extra.json下disableExtMakeup值为{}'.format(extra['settings']['disableExtMakeup']))
                else:
        ...
        with open(file) as f:
            ...
            for filter in filters:
                if '2d_sequence_resources' in filter and 'lash' in filter['2d_sequence_resources']['path']:
                    ...
                    eyelash.append('{}睫毛检查：path={}, filterType={}'.format(p, filter['2d_sequence_resources']['path'], filter['filterType']))
                    #print("睫毛类型：",filter['filterType'])
                    if filter['filterType']!="jiemao_faceu":
                        msg = "睫毛检查不通过，该睫毛类型为%s,应该修改为jiemao_faceu"%(filter['filterType'])
                        enmsg = "eyelash check does not pass, eyelash type is%s, please modify according to the standard"%(filter['filterType'])
                        effectLog.addEnglishErrorThing(msg)
            ...
            if('effect' in config_string):
                ...
                if('algorithmConfigs' in effect):
                    ...
                    if (ObjectTrack_vakue == 'ObjectTrack' and LooseVersion(effectzip.version) < '9.5.0'):
                        msg = "error file:%s ,资源包版本号为%s, 使用ObjectTrack算法的资源包版本需要≥9.5.0."%('config.json',effectzip.version)
                        enmsg = "error file:%s ,Config.json version number is %s, Please change the version number ≥ 9.5.0."%('config.json',effectzip.version)
                        effectLog.addEnglishErrorThing(msg)
                ...
                with open(fi) as f:
                    ...
                    if('requirement' in config_string):
                        ...
                        if ('ObjectTrack' in requirement and LooseVersion(effectzip.version) < '9.5.0'):
                            msg ="error file:%s ,资源包版本号为%s, 使用ObjectTrack算法的资源包版本需要≥9.5.0."%('content.json',effectzip.version)
                            enmsg = "error file:%s ,Config.json version number is %s, Please change the version number ≥ 9.5.0."%('config.json',effectzip.version)
                            effectLog.addEnglishErrorThing(msg)
...
def get_FileSize(effectzip,effectLog,param):
    ...
    if effect_size > max_zip_size:
        msg ="资源包体积过大 %s业务线,zip资源包最大为限制%s, 该资源包为%s,请压缩包大小"%(param,max_zip_size,effect_size)
        enmsg = "package size to large %sthe size limit%s, this effect size is %s,please modify it according to the standard"%(param,max_zip_size,effect_size)
        effectLog.addEnglishErrorThing(msg)
...
def checkTiktokVersion(effectzip,effectLog):
    ...
    if('config.json' in list_dir):
        if (LooseVersion(effectzip.version) >= '7.7.0' and LooseVersion(effectzip.version) <= '8.6.0'):
            msg = "error file:%s ,资源包版本号为%s, Tiktok无此版本号，请修正为8.7.0."%('config.json',effectzip.version)
            enmsg = "error file:%s ,Config.json version number is %s, Please change the version number to 8.7.0."%('config.json',effectzip.version)
            effectLog.addEnglishErrorThing(msg)
            effectLog.addJsonData(1,'VERSION_ERROR','config.json',-1,msg,enmsg)
        if (LooseVersion(effectzip.version) >= '7.3.0' and LooseVersion(effectzip.version) <='7.5.0'):
            msg = "error file:%s ,资源包版本号为%s, Tiktok无此版本号，请修正为7.6.0."%('config.json',effectzip.version)
            enmsg = "error file:%s ,Config.json version number is %s, Please change the version number to 7.6.0."%('config.json',effectzip.version)
            effectLog.addEnglishErrorThing(msg)
            effectLog.addJsonData(1,'VERSION_ERROR','config.json',-1,msg,enmsg) 
                    ...
                    if('Amaz.Guid' in file_string):
                        try:
                            ...
                            if (LooseVersion(effectzip.version) < '8.7.0'):
                                msg = "error file:%s ,资源包版本号为%s, 调用Amaz.Guid函数最低版本号需要≥8.7.0."%('config.json', effectzip.version)
                                enmsg = "error file:%s ,Config.json version number is %s, please change the version number ≥ 8.7.0."%('config.json', effectzip.version)
                                effectLog.addEnglishErrorThing(msg)
                                effectLog.addJsonData(1,'VERSION_ERROR','config.json',-1,msg,enmsg)
                        except:
                            pass   
...
def check_is_bach(effectzip,effectLog):
    ...
    if fi != None:
        with open(fi) as f:
            ...
            if('bALG_BACH_CONFIG' in Config_string and Config_string['bALG_BACH_CONFIG']==True):
                #print("bach")
                effect_version = Merge(EffectData.capcut_effect_requirement_version,EffectData.capcut_effect_bach_version,EffectData.capcut_effect_system_version)
            else:
                #print("not bach")
                effect_version = Merge_2(EffectData.capcut_effect_requirement_version,EffectData.capcut_effect_system_version)
...
def check_capcut_version(effectzip,effectLog):
    ...
    if LooseVersion(effectzip.version) < LooseVersion(max_version):
        msg ='资源包版本号配置错误，%s能力最低版本号限制%s，请将%s修改至%s及以上'%(max_version_effect,max_version,effectzip.version,max_version)
        enmsg = 'effect version error: %s version is %s, please change it≥%s'%(max_version_effect,max_version,max_version)
        effectLog.addJsonData(1,'VERSION_ERROR',effectzip.zip_name,-1,msg,enmsg)
            ...
            with open(di,encoding = 'utf-8') as di_path:
                ...
                for disable_keyword in EffectData.disable_keyword:
                    if disable_keyword in file_string:
                        ...
                        enmsg = 'The package has Disable Keyword,please open %s and change【%s】to SIDE_NOD'%(di,disable_keyword)
            ...
            for items in algorithm_string['nodes']:
                if('type' in items and 'blit' in items['type']):
                    ...
                    if('size' not in items['config'] and LooseVersion(effectzip.version) < '9.5.0'):
                        msg ='algorithmConfig.json文件中解析"type": "blit"，需要配置"size"或SDK版本大于等于950【%s】'%(effectzip.version)
                ...
                if('type' in items and 'expression_detect' in items['type']):
                    if('config' in items and 'keyMaps' in items['config']):
                        if('stringParam' in items['config']['keyMaps']):
                            if('face_attr_age_gender_model_dir' in items['config']['keyMaps']['stringParam']and 'face_attr_expression_model_dir' in items['config']['keyMaps']['stringParam']):
        ...
        with open(fi) as f:
            ...
            for items in algorithm_string['nodes']:
                if('type' in items and items['type']=='face' and 'stringParam' in items['config']['keyMaps']):
                    ...
                    #     print('1')
                    if (LooseVersion(effectzip.version) < '10.1.0') and ('face_base_model_key' in stringParam or 'face_extra_model_key' in stringParam):
...
def check_animatorControllerScript_version(effectzip,effectLog):
     for fi in effectzip.files:
        fis = fi.split('/')
        if(fis[-1]== 'animatorControllerScript' or fis[-2]== 'animatorControllerScript' or fis[-3]== 'animatorControllerScript'):
...
def AmazingScene_version(effectzip,effectLog):
   for fi in effectzip.files:
        ...
        if(fis[-1]== 'config.json'):
            with open(fi) as f:
                algorithm_string = json.loads(f.read())
                if('effect' in algorithm_string and 'AmazingScene' in algorithm_string['effect'] and 'asynLoad' in algorithm_string['effect']['AmazingScene']):
...
def check_modelname_version(effectzip,effectLog,url,access_key,secret_key):
    ...
    for v in modelVersions:
        if LooseVersion(effectzip.version) < v['minEffectVersion']:
            msg = '使用的modelname %s 最低版本为 %s，需要将资源包从%s提高至 %s'%(v['name'],v['minEffectVersion'],effectzip.version,v['minEffectVersion'])
            enmsg = 'modelname %s min version is %s，please change effect %s into %s'%(v['name'],v['minEffectVersion'],effectzip.version,v['minEffectVersion'])
            effectLog.addJsonData(1, 'VERSION_ERROR', 'config.json', -1,msg, enmsg)
...
def checkBachGraphScript(effectzip,effectLog):
    for fi in effectzip.files:
        ...
        if (fis[-1] == 'BachGraphScript.js'):
            with open(fi, 'r') as f:
                content = f.read()
                if re.search(r'algorthmManager.setAlgorithmEnable\("effectsdk_defaut_graph", "blit_0", false\);', content):
...
def TT_AmazingScene_version(effectzip,effectLog):
    ...
    with open(configList[MinLen_config_index],'r') as f:
        ...
        if('effect' in algorithm_string and 'AmazingScene' in algorithm_string['effect']) and (LooseVersion(effectzip.version) < '13.1.0'):
    ...
    for fi in effectzip.files:
        ...
        if (fis[-1] == 'sticker.json'):
            with open(fi) as f:
                algorithm_string = json.loads(f.read())
                if ('EffectNodeSystem' in algorithm_string['systemList']) and (LooseVersion(effectzip.version) < '13.1.0'):
                    msg = '道具中使用了新渲染链，需要sdk提升至13.1.0及以上'
                    enmsg = 'new render chain is used，please change sdk≥13.1.0'
                    effectLog.addJsonData(1, 'VERSION_ERROR', 'config.json', -1, msg, enmsg)
...
def checkModelNameIntegrity(effectzip,effectLog):
    ...
    modelNameKeys = ["audio_avatar_model_name","ava_boost_model_name","avatar_fit_model_name","avatar_fit_model_name","avatar_fit_model_name","car_series_model_name"," cloth_gan_model_key","compress_shot_detect_backbone_model_name","compress_shot_detect_predhead_model_name","content_recommend_model_key","deep_inpaint_model_key_coarse","deep_inpaint_model_key_fine","licensecake_detection_model_name","licensecake_detection_cake_landmark_model_name","licensecake_detection_person_recognition_model_name","licensecake_detection_cake_landmark_det_model_name","lm_3d_network_weights","lm_3d_mean_face_info","keypoint_name","navi_avatar_drive_model_key","action_detect_static_model_key","action_detect_sequence_model_key","action_recognition_tmpl_name","auto_detection_object_model_name","auto_detection_human_model_name"," avatar3d_model_key","avatar_drive_model_key"," blockgan_model_key","building_normal_scene_normal_model_path","building_normal_reg_model_path","building_seg_model_key","car_seg_model_key","cloth_class_model_key","clothset_model_key","clothseg_model_key","depth_model_key","face_attr_age_gender_model_dir","face_attr_expression_model_dir","face_attr_extra_model_dir","face_attr_model_dir","face_base_model_key","face_extra_model_key","face_extra_fast_model_name","face_beautify_model_key","face_clusting_model_key"," face_gan_model_key","face_mask_model_key","face_part_beauty_model_key","face_pet_detect_model_key","face_verify_model_key","female_gan_model_key","food_comics_model_key","foot_model","forehead_seg_model_key","gaze_estimation_model_key","ground_seg_model_key","hp_model_name","hair_flow_model_key"," hand_object_seg_track_model_name","hdr_net_model_key","human_parsing_model_key","idream_model_key","indoor_seg_model_name","joints1_model_key","joints2_model_key","license_plate_detect_model_key","manga_model_key","mug_model_key","multi_object_tracking_ar_model_key","nail_model_key_seg","nail_slam_normal_model_name","object_detect_model_key","object_detection2_model_name","object_tracking_model_name","saliency_model_key","scene_normal_model_key","scene_model_key","skeleton_model_key","skeleton_pose_3d_model_key_skel","skeleton_pose_3d_model_key_pose","skin_seg_model_key","skyseg_model_key","swap_live_model_name","swapperme_model_key","teeth_model_key","tracking_ar_model_key","video_sr_model_key","watch_tryon_model_name","model_name"]
    #默认模型名配置，如果算法节点type配置了，但是stringParam未配置模型，则需要根据type获取对应model默认model，判断默认model是否在modelListConfig中
    defaultModel = {'action_detect': ['tt_action_detection', 'tt_pose_detection'], 'after_effect': 'tt_after_effect', 'avacap': 'tt_avacap', 'avatar_3d': 'tt_avatar3dsticker', 'avatar_drive': 'tt_avatar_drive', 'avatar_score': ['tt_beauty_attr6', 'tt_face_beauty'], 'big_gan': 'tt_biggan', 'building_normal': 'bingo_building_normal', 'building_seg': 'bingo_building_seg', 'clothes_seg': 'tt_clothes_seg', 'content_recommend': 'tt_hashtag', 'depth': 'tt_depth_estimation', 'ear_seg': 'tt_earseg_kp', 'expression_detect': ['tt_face_attribute_age', 'tt_face_attribute_exp'], 'eye_fitting': 'tt_eyefitting', 'f_parsing': 'tt_f_parsing', 'face': ['tt_face', 'tt_face_extra'], 'face_attr': ['tt_face_attribute_age', 'tt_face_attribute_exp', 'tt_face_attribute_extra'], 'face_beautify': 'tt_facebeautify', 'face_clusting': 'tt_bigbrother', 'face_fitting': ['tt_facefitting1220', 'tt_facefitting1256', 'tt_facefitting845'], 'face_gan': 'tt_facegan_class', 'face_light': 'tt_face_light', 'face_new_landmark': 'tt_face_new_landmark', 'face_pet_detect': 'tt_petface', 'face_verify': 'tt_faceverify', 'facefitting_3d': 'tt_facefitting_3d', 'female_gan': 'female_gan', 'foot': 'tt_foot', 'freid': 'tt_freid', 'gender_gan': 'tt_gendergan', 'ground_seg': 'tt_ground_seg', 'hair': 'tt_hair', 'hair_flow': 'tt_hair_flow', 'hand': ['tt_hand_box_reg', 'tt_hand_det', 'tt_hand_gesture', 'tt_hand_kp', 'tt_hand_kp3d', 'tt_hand_lr', 'tt_hand_ring', 'tt_hand_seg'], 'havatar': ['tt_havatar_action_cls', 'tt_havatar_det', 'tt_havatar_lr_box', 'tt_havatar_lr_cls', 'tt_havatar_pose', 'tt_havatar_track'], 'hdr_net': 'tt_hdrnet_effect', 'head_fitting': ['tt_head3d_fitting_obj', 'tt_head3d_obj'], 'head_seg': 'tt_headseg', 'human_parsing': 'tt_human_parsing', 'idream': 'tt_idream', 'indoor_seg': 'tt_indoor_seg', 'manga': 'ap_manga', 'matting': ['tt_matting', 'tt_matting_video', 'tt_matting_subject', 'tt_matting_subjgpu', 'tt_matting_large'], 'memoji_match': ['tt_memoji_matchsafe_facial', 'tt_memoji_matchsafe_features', 'tt_memoji_matchsafe_glasses', 'tt_memoji_matchsafe_hair', 'tt_memoji_matchsafe_sun'], 'nail': ['tt_nail_kpts', 'tt_nail_seg'], 'navi_avatar_drive': 'navi_avatar_drive', 'neck': 'tt_neck_pose', 'object_tracking': 'bingo_objecttracking', 'old_gan': 'tt_old', 'porn_cls': 'tt_porn_classifier', 'saliency_seg': 'bingo_saliency_seg', 'salient_human': 'tt_salient_human', 'scene_normal': 'bingo_scene_normal', 'scene_recog_v3': 'tt_c3_cls', 'scene_recognition': ['tt_c1_det', 'tt_c1_small'], 'similarity': 'tt_sim', 'skeleton': 'tt_skeleton', 'skin_unified': ['tt_facebeautify', 'tt_skin_unified'], 'sky_seg': 'tt_skyseg', 'slam_nail': 'ttslammodel', 'smash_matting': ['tt_matting', 'tt_matting_video'], 'stop_motion': ['tt_matting', 'tt_matting_video'], 'swapperme': ['tt_face', 'tt_facebeautify', 'tt_facefitting1220'], 'video_cls': 'tt_videocls', 'video_matting': 'tt_matting', 'watch_tryon': 'tt_watch_tryon'}
...
def checkEffectZip(effectzip,effectLog,param,lokiInformation=None):
    ...
    if(param == 'DEFAULT'):
        ...
        check_modelname_version(effectzip, effectLog,url = 'https://model-dispatcher.bytedance.net/openapi/model/getModelMinimumVersion',access_key = '4269ab2e88c8657c284cae87d55e41a5',secret_key = '9dcb30681e192565b32bbc9d628d8873')
        ...
        check_algorithm_expression_model(effectzip,effectLog)
        check_algorithm_face(effectzip,effectLog)
        check_modelname_version(effectzip, effectLog,url = 'https://model-dispatcher-us.byteintl.net/openapi/model/getModelMinimumVersion',access_key = '727280b4edb9892605848e1988359f0a',secret_key = '038b392d4f2b1d05fe84db2aaf6a245e')
        check_algorithmConfig_nh_inference(effectzip,effectLog)
        checkBachGraphScript(effectzip,effectLog)
